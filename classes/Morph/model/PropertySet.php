...
class  Morph_PropertySet extends ArrayObject
{
    ...
    {
        $state = Morph_Enum::STATE_NEW;
        $hasCleanProperty = false;
        $hasNewProperty = false;

        foreach ($this as $n => $property) {
            $propertyState = $property->getState();
            if ( Morph_Enum::STATE_DIRTY == $propertyState) {
                $state = $propertyState;
                break;
            } elseif ( Morph_Enum::STATE_CLEAN == $propertyState) {
                $state = $propertyState;
                $hasCleanProperty = true;
            } elseif ( Morph_Enum::STATE_NEW == $propertyState) {
                $state = $propertyState;
                $hasNewProperty = true;
            }
        }
    ...
    {
        throw new RuntimeException("Appending to Morph_PropertySet is not supported");
    }
    ...
    private function checkType($object)
    {
        ...
        if(!($object instanceof Morph_Property_Generic)){
            throw new InvalidArgumentException('object of type' 
                            . get_class($object) . ' does not extend Morph_Property_Generic');
        }
    }
}
